...
export function Leaderboard({ players, myId }: LeaderboardProps) {
  ...
  return (
    <div className="flex gap-1.5 overflow-x-auto border-b border-[var(--border)] bg-[var(--surface)] px-4 py-2.5">
      {entries.map(([id, player]) => {
        const avatar = getAvatar(player.avatarIndex);
        return (
          <div
            key={id}
            className={`flex items-center gap-1.5 whitespace-nowrap rounded-lg px-2.5 py-1.5 text-xs ${
              id === myId
          ...
          >
            ...
            <span className="max-w-[60px] truncate text-[var(--text-muted)]">
              {player.name}
            </span>
